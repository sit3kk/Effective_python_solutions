...
# Display current time by continent 
for continent, countries in countries_by_continent.items():
    print(f"Continent: {continent}")
    for country in countries:
        timezone = pytz.timezone(country['timezones'][0])
        country_time = datetime.now(timezone)
        print(f"  {country['name']} ({country['capital']}): {country_time.strftime('%Y-%m-%d %H:%M:%S')}")

def is_midnight(timezone):
...
while True:
    ...
    for country in countries:
        ...
        if is_midnight(timezone):
            print(f"It's now midnight in {country['name']}, {country['capital']}.")
            midnight_found = True
    
    if not midnight_found:
        print("It's not midnight anywhere in the world right now.")
